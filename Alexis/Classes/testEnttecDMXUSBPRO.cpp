...
int main(int argc, char *argv[])
{
    ...
    if(test)//Si l'interface DMX est OK
    {
		...
		//On initialise le tableau des valeur a envoyer par voie DMX Ã  0
		int valeurDMX[TAILLEBUSDMX];//tableau contenant les Valeur des 512 canaux du bus DMX
		memset(valeurDMX,0x00, TAILLEBUSDMX);

		//On crÃ©e et configure le socket en UDP avec le bon port et n'importe qu'elle interface reseau du raspberry(ici on a la clÃ© wi fi sur un rÃ©seau et la carte reseau sur un autre)
		...
		while(1)
		{
			memset(message, 0x00, LG_MESSAGE*sizeof(char));//On le remet Ã  0 Ã  chaque fois
			...
			//On rempli un tableau en 2 dimension avec les information decodÃ©es du message qu'on trie
			...
			//string testProjo = ADRESSEDEBUTPROJO;
			//string testLyre = ADRESSEDEBUTLYRE;
			//cout << "testProjo = " << testProjo << " ; testLyre = " << testLyre << "\nCible = " << cible << endl;
			...
			for(int i=0; i < fullDecoded.size();i=i+1)
				...
				for(int i=0; i<=3;i=i+1)//La lyre est divisÃ©e en 4 quart, on veut allumer la barre entierrement donc on fait une boucle qui allume chaque quart
				{
